...
async function main() {
  ...
  const adapterAddress =
    process.argv[2] || "0xe7f1725E7734CE288F8367e1Bb143E90bb3F0512";

  console.log(`📍 Adapter Address: ${adapterAddress}`);

  try {
    // Connect to the contract
    const priceFeedAdapter = await ethers.getContractAt(
      "PriceFeedAdapter",
      adapterAddress,
    );
    ...
    console.log(
      `💰 Account balance: ${ethers.formatEther(await signer.provider.getBalance(signer.address))} ETH`,
    );
    ...
    try {
      ...
      console.log(
        `Last Batch Height: Unable to fetch (${error instanceof Error ? error.message : "Unknown error"})`,
      );
    }
    ...
    try {
      ...
      console.log(
        `   ❌ Price decoding failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
    ...
    try {
      ...
      const resultVerifiedPromise = new Promise((resolve, reject) => {
        ...
        const resultVerifiedFilter =
          priceFeedAdapter.getEvent("ResultVerified");
        const verificationFailedFilter =
          priceFeedAdapter.getEvent("VerificationFailed");

        priceFeedAdapter.once(
          resultVerifiedFilter,
          (requestId, symbol, price, batchHeight, batchSender) => {
      ...
      const tx = await priceFeedAdapter.submitResult(
        mockResult,
        mockBatchHeight,
        mockMerkleProof,
      );
      ...
      // Wait for events
      try {
        const eventData = (await resultVerifiedPromise) as {
          requestId: string;
          symbol: string;
          price: bigint;
          batchHeight: bigint;
        ...
        console.log(
          `   ⚠️  Event handling: ${eventError instanceof Error ? eventError.message : "Unknown error"}`,
        );
      }
    } catch (error: unknown) {
      console.log(
    ...
    try {
      ...
      console.log(
        `   ❌ Owner function test failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
    ...
    console.log(
      "   4. Integrate with external systems via verification events",
    );
