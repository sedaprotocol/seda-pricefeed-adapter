...
async function main() {
  ...
  const globalId = ethers.keccak256(
    abiCoder.encode(["bytes32", "bytes32", "bytes32"], [EXEC_PROGRAM_ID, TALLY_PROGRAM_ID, rawId]),
  );
  ...
  console.log(`  publishTime: ${price.publishTime} (${new Date(Number(price.publishTime) * 1000).toISOString()})`);
